"""Small immutable-artifact fingerprint and Parquet-schema checks."""
...
def file_fingerprint(path: str | Path, *, relative_to: str | Path | None = None) -> dict[str, Any]:
    ...
    if relative_to is not None:
        stored_path = Path(source.relative_to(Path(relative_to).expanduser().resolve()))
    return {
...
def verify_fingerprint(record: Any, *, base_dir: str | Path | None = None) -> Path:
...
def normalized_schema(schema: Iterable[tuple[str, str]]) -> tuple[tuple[str, str], ...]:
...
def parquet_schema(path: str | Path) -> tuple[tuple[str, str], ...]:
    ...
    con = duckdb.connect()
    try:
        return tuple((row[0], row[1].upper()) for row in con.execute(
...
def require_parquet_schema(path: str | Path, schema: Iterable[tuple[str, str]], label: str) -> None:

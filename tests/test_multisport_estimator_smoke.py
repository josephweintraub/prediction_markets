...
def test_combined_estimator_writes_complete_fixed_grids(tmp_path: Path) -> None:
    ...
    _write_parquet(
        ...
        [("nhl", "nhl-event", "nhl-market", day, "pregame", 1,
          0.05, False, -0.05, 1.0, "new-wallet", day)],
    )
    _write_parquet(
        ...
        [
            ("nhl", "nhl-event", "nhl-market", day, sample,
    ...
    phase_count = sum(len(phases) for phases in LEGACY_PHASES.values()) + sum(
        len(config.phases) for config in SPORT_CONFIGS.values()
    )
    sport_count = len(LEGACY_PHASES) + len(SPORT_CONFIGS)
    expected_phase_rows = phase_count * 10
    expected_closing_rows = sport_count * 2 * 10
    expected_spread_rows = phase_count + sport_count * 2
    assert (expected_phase_rows, expected_closing_rows, expected_spread_rows) == (460, 220, 68)

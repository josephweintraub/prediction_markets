...
- Embedding-difficulty sessions 6/6b (`make_liq_horizon_slices.py`,
  `make_timeliq_slices.py`): liquidity re-measured as volume RATE ($/day of market life)
  and as fixed-window volumes (first 1d/7d/30d, anchored at first filtered trade);
  quartile schemes pooled/within-vintage; horizon × within-bin-quartile crosses;
  clean-maturity samples (standalone binaries; final-two per multi-market event with
  dropout-contamination stats). Report v6 section 5c, decile-first.

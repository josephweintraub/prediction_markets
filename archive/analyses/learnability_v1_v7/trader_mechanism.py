"""Who makes the miscalibration? Trader-composition mechanism tests.

Two designs separating "places make bias" from "people carry bias":

A. WITHIN-WALLET horse race: wallet fixed effects x [1, price] absorbed, so
   the coefficient on p*x_d asks whether the SAME wallet's trades are more
   miscalibrated when it trades a tail/thin/long market. If yes -> the market
   causes it; if zero -> selection of people into markets.

B. PARTICIPANT SPLIT: classify wallets by participation in the biased cells
   (>=5 mature trades in novelty-tail markets; separately, in thin (q1-2)
   markets). Measure their calibration in the OTHER (non-tail / thick)
   markets vs non-participants. If participants are biased everywhere ->
   trader trait; if calibrated elsewhere -> market-structure story.

Plus overlap descriptives. Sample: horse-race estimation sample (mature).
Outputs: trader_mechanism_*.parquet under /mnt/data/learnability/output/.
"""
...
def slope_row(sub, label):
    ...
    bd, sed = threeway_cluster_slope(y, sub["price"], sub["usdc"],
                                     ...
                                     sub["market_id"])
    log(f"  {label:44s} n={len(sub):>10,} dev={b:+.4f} "
...
def main():
    ...
    df = con.execute(f"""
        SELECT t.price, t.won, t.usdc, t.trade_day, t.proxyWallet, t.market_id,
               d.life_d, d.usd_full, (d.novelty_vint_decile = 1) AS nov_tail
    ...
    """).fetchdf()
    log(f"sample: {len(df):,} trades, {df['proxyWallet'].nunique():,} wallets")

    # market-level z dims (same convention as horse_race_v1)
    ...
    z_usd = np.log1p(mk["usd_full"])
    liq_q = np.ceil(z_usd.rank(pct=True) * 5).clip(1, 5)
    z_usd = (z_usd - z_usd.mean()) / z_usd.std()
    ...
    # A. within-wallet joint model (all wallets, then active >=20 trades)
    log("=== A: within-wallet (wallet FE x [1,p]) ===")
    frames = [joint_model(df.assign(_fe=df["proxyWallet"]), "_fe", None,
                          W_DIMS, "walletFE|count|all")]
    ntr = df.groupby("proxyWallet", observed=True)["price"].transform("size")
    act = df[ntr >= 20]
    log(f"  active-wallet subsample: {len(act):,} trades, "
        f"{act['proxyWallet'].nunique():,} wallets")
    frames.append(joint_model(act.assign(_fe=act["proxyWallet"]), "_fe", None,
                              W_DIMS, "walletFE|count|ge20"))
    frames.append(joint_model(act.assign(_fe=act["proxyWallet"]), "_fe",
                              "usdc", W_DIMS, "walletFE|dollar|ge20"))
    pd.concat(frames, ignore_index=True).to_parquet(
        OUT / "trader_mechanism_walletfe.parquet")

    # B. participant splits
    log("=== B: participant splits ===")
    rows = []
    tail_ct = (df[df["nov_tail"] == 1].groupby("proxyWallet", observed=True)
               .size())
    thin_ct = (df[df["liq_q"] <= 2].groupby("proxyWallet", observed=True)
               .size())
    ...
    pd.DataFrame(rows).to_parquet(OUT / "trader_mechanism_splits.parquet")

    # descriptives
    log("=== overlap descriptives ===")
    ...
    desc = {
        ...
        "median_trades_ge5": float(ntr[is5].median()),
        "median_trades_others": float(ntr[~is5].median()),
    }
    pd.DataFrame([desc]).to_parquet(OUT / "trader_mechanism_desc.parquet")
    log(str(desc))
    log(f"DONE trader_mechanism in {(time.time()-t0)/60:.1f} min")
